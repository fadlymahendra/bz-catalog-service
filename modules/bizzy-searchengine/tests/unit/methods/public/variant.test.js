...
const Methods = require('../../../../src/methods/public/variant');
...
test.serial('Should be return success', function* (t) {
    ...
    t.context.sandbox.stub(ProductVendorRepository, 'findAndCountAll').resolves({
        rows: [
            {
                id: 2,
    ...
    try {
        const result = yield Methods.getPublicSku(datas, context);
...
test.serial('Should be return success NO_VARIANT', function* (t) {
    resultFindOneProVar.variant_value = 'NO_VARIANT';
    t.context.sandbox.stub(ProductVariantRepository, 'findOne').resolves(resultFindOneProVar);
    t.context.sandbox.stub(ProductGroupRepository, 'findById').resolves(resultFindIdPG);
    t.context.sandbox.stub(ProductVendorRepository, 'findAndCountAll').resolves({
        rows: [
    ...
    try {
        const result = yield Methods.getPublicSku(datas, context);
        expected.data.variant_value = 'NO_VARIANT';
...
test.serial('Should be return Bad Request', function* (t) {
    ...
    try {
        ...
        yield Methods.getPublicSku(data, context);
...
test.serial('Should be return Vendor Not Found', function* (t) {
    ...
    t.context.sandbox.stub(ProductSkuMappingRepository, 'findMappingSku').resolves([]);
    t.context.sandbox.stub(ProductVariantRepository, 'findOne').resolves(resultFindOneProVar);
    t.context.sandbox.stub(ProductVendorRepository, 'findAndCountAll').resolves({ count: 0 });
    try {
        const data = {
            path: {
                id: 'X152KT2GKFSDF'
