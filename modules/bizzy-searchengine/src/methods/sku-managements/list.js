...
exports.getSkuManagement = async function (data, context) {
    ...
    const rows = await RepoRaw.getSKUProducts(wheres, offset, limit);
    const total = await RepoRaw.getSKUProducts(wheres);
    ...
    let totalVendor = [];
    if (rows.length) { totalVendor = await RepoRaw.getTotalVendor(ids); }
