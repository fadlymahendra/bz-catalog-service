...
const Methods = require('../../../src/methods/warranty_period');

const sandbox = sinon.sandbox.create().usingPromise(Promise.Promise);

test.serial('Should be return Warranty Period List', function*(t) {
    ...
    try {
        ...
        const result = yield Methods.getWarrantyPeriod(data, context);
        const expected = {
            data:{
                "value": "week",
                "label": "Minggu"    
...
test.serial('Should be return authorized user', function*(t) {
    try {
        ...
        yield Methods.getWarrantyPeriod(data, context);
