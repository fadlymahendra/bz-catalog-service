...
test.serial('Should return category list C0 with child', function* (t) {
    const f = t.context.sandbox.stub(CategoryRepository, 'findAll');
    f.onCall(0).resolves([
        {
            id: 23,
            name: 'b',
    ...
    try {
        yield Methods.getCategoryAll(payload, context);
        t.pass();
    } catch (err) {
...
test.serial('Should return category list C0 with child, return Forbiden', function* (t) {
    const f = t.context.sandbox.stub(CategoryRepository, 'findAll');
    f.onCall(0).resolves([
        {
            id: 23,
    ...
    try {
        yield Methods.getCategoryAll(payload, {});
        t.fail('Should return category list with child, return Forbidden');
    } catch (err) {
...
test.serial('Should return category detail BadRequest, invalid payload', function* (t) {
    t.context.sandbox.stub(CategoryRepository, 'findOneWithParent').resolves(null);
    const payload = {
        path: {
            ids: '1'
        }
...
test.serial('Should return category detail BadRequest, invalid context', function* (t) {
    t.context.sandbox.stub(CategoryRepository, 'findOneWithParent').resolves(null);
    const payload = {
        path: {
            id: '1'
    ...
    try {
        yield Methods.getCategoryDetail(payload, {});
        t.fail('Should return category detail BadRequest, invalid payload');
    } catch (err) {
        t.true(err instanceof BizzyError.Forbidden);
        t.pass();
