...
test.serial('Should be return nothing', function* testCase(t) {
    ...
    try{
        ...
        const result = yield Methods.deleteUom(data, context);
        const expected = '';
...
test.serial('Invalid Delete Should throw BizzyError.InternalServerError', function* testCase(t) {
    t.context.sandbox.stub(UomRespository, 'findById').resolves(
        {"id":1, "name":"Unit", 'created_at':'2017-12-05 16:59:10', 'updated_at':null}
    );    
    t.context.sandbox.stub(UomRespository, 'delete').resolves(false);
    try{
        let context = require('../../../mocks/context.json');
        const data = {
            path: {
...
test.serial('You are not authorized user: Should be return Forbidden', function* (t) {
    try {
        ...
        yield Methods.deleteUom(data, context);
        t.fail('The validator doesn\'t throw an error');
    } catch(err) {
