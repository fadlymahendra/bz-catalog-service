...
const Methods = require('../../../../src/methods/mails/premoderation_assign');

const sandbox = sinon.sandbox.create().usingPromise(Promise.Promise);

test.serial('Should be return send email', function* (t) {
    t.context.sandbox.stub(PremoderationRepository, 'findAllGroupAssign').resolves([
        {
            _id: 3,
            data: [
                {
    ...
    try {
        ...
        const result = yield Methods.postEmailPremoderationAssign(data, context);
        const expected = {
